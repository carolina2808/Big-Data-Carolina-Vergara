...
# Convertir fechas de DD/MM/YY a YYYY-MM-DD y reemplazar ; por ,
sed -E '
    s/([0-9]{2})\/([0-9]{2})\/([0-9]{2})/20\3-\2-\1/g;
    s/\./,/g;
    s/;/,/g;
    s/(,\N)(,|$)/\1/g;
    s/(,[0-9]+),/\1,/g' data.csv | while read -r line; do
    if [[ "${line: -2}" != "\N" ]]; then
        echo "${line%?},${line: -1}"
    else
        echo "$line"
...
# Reemplazar las dobles comas (,,) por punto (,)
sed -E 's/,,/./g' temp.csv > output.csv
